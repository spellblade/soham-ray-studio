    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          console.error("[app-builder] /api handler failed:", err);
          if (!res.headersSent) {
            res.statusCode = 500;
 ...
 * This middleware runs before the SPA HTML fallback, calls
 * `handleAuthPopupRequest`, and returns the 302 / completion HTML. Deployed
 * apps do not use the popup (full-page OAuth redirect), so `apply: "serve"`
 * is enough.
 */
      ...
      server.middlewares.use(async (req, res, next) => {
        try {
          ...
          const mod = (await server.ssrLoadModule("/src/lib/auth/popup.server.ts")) as {
            handleAuthPopupRequest: (incoming: Request) => Promise<Response>;
          };
